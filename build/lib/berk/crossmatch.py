...
def compute_LR_Rel(radio_sources, optical_sources, search_radius_asec, opt_mag_col, mag_bins, Q0, qm_nm_values, radRACol, radDecCol, optRACol, optDecCol, eRadRACol, eRadDecCol, optPosErrCol):
	...
	for radio_idx, optical_indices in enumerate(matches):
		...
		Rels_radio_source = [LR / denominator for LR in LRs_radio_source] # List of Rels of all matches for a given radio source
...
def xmatchOpt(catalog, opt_survey, opt_survey_dr, opt_mag_col, search_radius_asec, outPath, makePlots, radRACol, radDecCol, eRadRACol, eRadDecCol, outSubscript, saveFiles=True):
	...
	if(saveFiles):
		optical_sources.write(outPath+os.path.sep+'DECaLS_sources_%s.fits' %outSubscript, format='fits', overwrite=True)
		rand_radio_sources.write(outPath+os.path.sep+'Randoms_%s.fits' %outSubscript, format='fits', overwrite=True)
		np.savetxt(outPath+os.path.sep+'Q0_%s.txt' %outSubscript, [Q0], fmt='%f')
...
def xmatch_berk(radio_cat, radio_band, xmatchTabOutName, opt_survey='DECaLS', opt_survey_dr='DR10', opt_mag_col = 'r', search_radius_asec = 4.0, makePlots=False, radRACol='RA', radDecCol='DEC', eRadRACol='E_RA', eRadDecCol='E_DEC', outSubscript=''):
	...
	if(opt_survey == 'DECaLS'):
		...
		try:
			xmatch_tab = xmatchOpt(catalog=radio_cat, opt_survey=opt_survey, opt_survey_dr=opt_survey_dr, opt_mag_col=opt_mag_col, search_radius_asec=search_radius_asec, outPath=xmatchDirPath, makePlots=makePlots, radRACol=radRACol, radDecCol=radDecCol, eRadRACol=eRadRACol, eRadDecCol=eRadDecCol, outSubscript=outSubscript, saveFiles=False)
